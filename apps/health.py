...
from shared.db import (
    ...
    export_sales_df, export_products_df, export_expenses_df,
...
from shared.theme import (
    apply_suite_css, kpi_card, kpi_dashboard, section_header, page_header,
  ...
  <div style="text-align:right;font-size:0.82rem;color:#8BA0B8;">
    ...
    with tab5:
        ...
        def _export_csv_bytes(df: pd.DataFrame) -> bytes:
            # Business-friendly CSV body + a plain footer line for branding.
            # Purely cosmetic — not a security control — so it never blocks
            # the export if anything about the business name is missing.
            biz_name = user.get("business_name", "your business")
            footer   = f"\nGenerated by BizTrack-OS — {biz_name} — {datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
            return (df.to_csv(index=False) + footer).encode("utf-8")

        col1, col2, col3 = st.columns(3)
        with col1:
            if not sales_df.empty:
                st.download_button("⬇️ Download Sales CSV",
                                   data=_export_csv_bytes(export_sales_df(business_id)),
        ...
        with col2:
            ...
            if not products_df.empty:
                st.download_button("⬇️ Download Products CSV",
                                   data=_export_csv_bytes(export_products_df(business_id)),
                                   file_name="products_export.csv", mime="text/csv",
                                   width='stretch')
        with col3:
            if not expenses_df.empty:
                st.download_button("⬇️ Download Expenses CSV",
                                   data=_export_csv_bytes(export_expenses_df(business_id)),
...
def page_admin():
    ...
    if snapshot_view == "🏠 General":
        ...
        if payments_df.empty:
            ...
            kpi_dashboard(
                title=f"{'🇳🇬 Nigeria (NGN)' if is_ngn else '🌍 Global (USD)'} — Actual Collected",
                metrics=[
                    {"icon": "💰", "label": "All-Time Revenue", "value": f"{symbol}{total:,.0f}", "sub": f"{txns} payments"},
                    {"icon": "📅", "label": "This Month",       "value": f"{symbol}{month:,.0f}", "sub": now_dt.strftime("%B %Y")},
                    {"icon": "🗓️", "label": "This Year",        "value": f"{symbol}{year:,.0f}",  "sub": str(now_dt.year)},
                    {"icon": "🧾", "label": "Avg. per Payment", "value": f"{symbol}{avg:,.0f}",   "sub": f"Across {label} activations"},
                ],
                columns=2,
            )
    ...
    with tab3:
        if not payments_df.empty:
            ...
            def _mrr_bar_chart(df, symbol, color):
            ...
            with st.expander("🇳🇬 NGN Revenue Growth", expanded=True):
                if ngn_df.empty:
                    ...
                    st.plotly_chart(_mrr_bar_chart(ngn_df, "₦", CHART_GOLD), width='stretch')

            with st.expander("🌍 USD Revenue Growth", expanded=True):
                if usd_df.empty:
                    st.info("No USD payment data yet.")
                else:
                    st.plotly_chart(_mrr_bar_chart(usd_df, "$", CHART_JADE), width='stretch')
        else:

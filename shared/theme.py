...
def kpi_dashboard(metrics: list[dict], columns: int = 2, title: str = ""):
    ...
    Value font-size uses clamp() rather than a fixed rem — on a narrow
    2-column mobile cell a fixed 1.3rem left no room for amounts like
    ₦698,550.00, so word-break:break-word split them mid-digit onto a
    second line. clamp() shrinks the value to fit the available width
    instead, and only falls back to wrapping for genuinely long values.
    """
    ...
    for m in metrics:
        ...
        sub_html = f'<div style="font-size:0.72rem; color:var(--text-muted); margin-top:0.3rem;">{m["sub"]}</div>' if m.get("sub") else ""
        ...
        cells += (
            '<div style="background:var(--surface2); border-radius:10px; padding:0.95rem 1.05rem;">'
            f'<div style="font-size:0.7rem; color:var(--text-secondary); letter-spacing:0.03em; text-transform:uppercase; margin-bottom:0.45rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{icon_html}{m["label"]}</div>'
            f'<div style="font-family:var(--font-display); font-size:clamp(1rem, 5vw, 1.2rem); font-weight:800; color:var(--text-primary); line-height:1.25; word-break:break-word;">{m["value"]}</div>'
            f'{sub_html}'
            '</div>'
        )
    st.markdown(
        ...
        f'{title_html}'
        f'<div style="display:grid; grid-template-columns:repeat({columns}, 1fr); gap:12px;">{cells}</div>'
        '</div>',
        unsafe_allow_html=True,
    )
